...
void stickman(int speed,int color)
{
	int s=speed;
	...
	{
	setcolor(color);
	circle(20+s,290,30+i);
	line(20+i+s,320,20+i+s,370);
	//legs
	line(20+i+s,370,i+s,420);
	line(20+i+s,370,40+i+s,420);
	//left hand
		line(20+s+i,330,-10+s+i,370);
	//right hand
		line(20+s,330+i,50+s,370+i);
	}
...
void stickwomen_gift(int speed,int color)
{
	...
	line(s+i+10,353,s+i+10,337);
	line(s+i-10,353,s+i+10,353);
	line(s+i-10,353,s+i-10,337);
	line(s+i-10,337,s+i+10,337);
	}
	setcolor(LIGHTRED);
	for(int i=10,h=0;i>=0;i--,h++){
...
void stickman_chat(int speed,int box_color,int text_color)
{
	...
	int points[16]={50+s,280,40+s,270,-40+s,270,-40+s,200,180+s,200,180+s,270,60+s,270,50+s,280};
	drawpoly(8,points);

   setcolor(text_color);
    settextstyle(EUROPEAN_FONT,HORIZ_DIR,2);
	outtextxy(-20+s,210,"Happy Birthday ");
	outtextxy(-5+s,240," Aishwarya Di!");
