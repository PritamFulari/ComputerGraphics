...
void stickwomen_chat(int speed,int box_color,int text_color)
{
   ...
   setcolor(text_color);
    settextstyle(EUROPEAN_FONT,HORIZ_DIR,1);
	outtextxy(45+s,210,"Sorry I ");
	outtextxy(100+s,240,"hate u");
...
{setcolor(LIGHTGRAY);
 int a=speed;
	...
	setfillstyle(7,cloud_color);
	floodfill(50+a,75,LIGHTGRAY);
}
// wind
void wind(int speed,int wind_size)
{
   ...
   setcolor(WHITE);
	...
	for(int i=0;i<8;i++)
	{
		settextstyle(0,HORIZ_DIR,wind_size);
...
//	    	stickman_open(345,14);
	...
	//stick women walk 70
		...
		if(n>70&&n<=170)
		{
			...
			if(n%2==0){
//			PlaySound(TEXT("res/.mp3"),NULL,SND_SYNC);
